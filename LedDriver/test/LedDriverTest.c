...
{
	PCU_ASSERT_TRUE(LedDriver_IsOff(0));
	PCU_ASSERT_TRUE(LedDriver_IsOff(17));
	PCU_ASSERT_FALSE(LedDriver_IsOn(0));
	PCU_ASSERT_FALSE(LedDriver_IsOn(17));
}

static void test_IsOff(void)
{
	PCU_ASSERT_TRUE(LedDriver_IsOff(12));
	LedDriver_TurnOn(12);
	PCU_ASSERT_FALSE(LedDriver_IsOff(12));
}

static void test_TurnOffMultipleLeds(void)
{
	LedDriver_TurnAllOn();
	LedDriver_TurnOff(9);
	LedDriver_TurnOff(8);
	PCU_ASSERT_EQUAL((uint16_t) ((~0x180) & 0xffff), virtualLeds);
}
...
PCU_Suite *LedDriverTest_suite(void)
{
	static PCU_Test tests[] = {
		...
		PCU_TEST(test_IsOff),
		PCU_TEST(test_TurnOffMultipleLeds),
		PCU_TEST(test_AllOff),
	};
